...
### 3. ローカル文字起こし（Voxtral）の設定（オプション）

インターネット接続なしで音声認識を行いたい場合、Voxtralローカルモデルを使用できます。

**前提条件**:
- Apple Silicon Mac（推奨）
- Python 3.10+ と `uvx` がインストール済み

**設定方法**:
1. 設定 → 「API」タブ
2. 「ローカル文字起こし」セクションのトグルを有効化
3. ホスト（デフォルト: `127.0.0.1`）とポート（デフォルト: `8000`）を確認
4. 「Start Server」ボタンをクリック
5. サーバー状態が「Loading model...」→「Ready」に変わるまで待機（初回はモデルダウンロードのため時間がかかります）
6. 「Test Connection」ボタンで接続を確認

**注意**: ローカルモード使用時でもGemini APIキーは必要です（テキスト編集・コマンドモード・ルール処理で使用）。
...
**ローカルモード（Voxtral）有効時**: 録音中にリアルタイムでストリーミング文字起こしが行われ、オーバーレイに中間テキストが表示されます。
...
- マイク権限が許可されているか確認
- インターネット接続を確認（Gemini APIモード時）
- Gemini API の利用制限に達していないか確認

### ローカル文字起こしが動作しない

- サーバー状態が「Ready」になっているか確認
- 「Test Connection」ボタンで接続を確認
- `uvx` コマンドがPATHに存在するか確認（ターミナルで `which uvx` を実行）
- ポートが他のアプリと競合していないか確認（別のポート番号を試す）
